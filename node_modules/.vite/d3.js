...
function axis(orient, scale2) {
  ...
  function axis2(context) {
    var values = tickValues == null ? scale2.ticks ? scale2.ticks.apply(scale2, tickArguments) : scale2.domain() : tickValues, format2 = tickFormat2 == null ? scale2.tickFormat ? scale2.tickFormat.apply(scale2, tickArguments) : identity_default2 : tickFormat2, spacing = Math.max(tickSizeInner, 0) + tickPadding, range3 = scale2.range(), range0 = +range3[0] + offset, range1 = +range3[range3.length - 1] + offset, position = (scale2.bandwidth ? center : number)(scale2.copy(), offset), selection = context.selection ? context.selection() : context, path = selection.selectAll(".domain").data([null]), tick = selection.selectAll(".tick").data(values, scale2).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line = tick.select("line"), text = tick.select("text");
...
function density_default() {
  var x = defaultX, y = defaultY, weight = defaultWeight, dx = 960, dy = 500, r = 20, k = 2, o = r * 3, n = dx + o * 2 >> k, m = dy + o * 2 >> k, threshold2 = constant_default3(20);
...
function encloseBasis3(a, b, c) {
  var x1 = a.x, y1 = a.y, r1 = a.r, x2 = b.x, y2 = b.y, r2 = b.r, x3 = c.x, y3 = c.y, r3 = c.r, a2 = x1 - x2, a3 = x1 - x3, b2 = y1 - y2, b3 = y1 - y3, c2 = r2 - r1, c3 = r3 - r1, d1 = x1 * x1 + y1 * y1 - r1 * r1, d2 = d1 - x2 * x2 - y2 * y2 + r2 * r2, d3 = d1 - x3 * x3 - y3 * y3 + r3 * r3, ab4 = a3 * b2 - a2 * b3, xa = (b2 * d3 - b3 * d2) / (ab4 * 2) - x1, xb = (b3 * c2 - b2 * c3) / ab4, ya = (a3 * d2 - a2 * d3) / (ab4 * 2) - y1, yb = (a2 * c3 - a3 * c2) / ab4, A = xb * xb + yb * yb - 1, B2 = 2 * (r1 + xa * xb + ya * yb), C = xa * xa + ya * ya - r1 * r1, r = -(A ? (B2 + Math.sqrt(B2 * B2 - 4 * A * C)) / (2 * A) : C / B2);
  return {
    x: x1 + xa + xb * r,
    y: y1 + ya + yb * r,
    r
...
function score(node) {
  var a = node._, b = node.next._, ab4 = a.r + b.r, dx = (a.x * b.r + b.x * a.r) / ab4, dy = (a.y * b.r + b.y * a.r) / ab4;
  return dx * dx + dy * dy;
}
...
function packEnclose(circles) {
  if (!(n = (circles = array_default2(circles)).length))
    return 0;
  var a, b, c, n, aa2, ca3, i, j, k, sj, sk;
