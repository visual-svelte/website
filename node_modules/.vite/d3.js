...
} from "./chunk-7HZFYUXX.js";
import {
  drag_default,
  nodrag_default,
  yesdrag
} from "./chunk-MH55JJUR.js";
import {
  interval_default,
  now,
  timeout_default,
  timer,
  timerFlush
} from "./chunk-2T2OG6YQ.js";
import {
  dispatch_default
} from "./chunk-MJBEG7XQ.js";
import {
  Accent_default,
  Blues_default,
  BrBG_default,
...
function density_default() {
  var x2 = defaultX, y2 = defaultY, weight = defaultWeight, dx = 960, dy = 500, r = 20, k2 = 2, o = r * 3, n = dx + o * 2 >> k2, m = dy + o * 2 >> k2, threshold2 = constant_default3(20);
  function density(data) {
    var values0 = new Float32Array(n * m), values1 = new Float32Array(n * m), pow2k = Math.pow(2, -k2);
  ...
  _project(x06, y06, vx, vy) {
    ...
    if (vy < 0) {
      ...
      if ((c2 = (this.ymin - y06) / vy) < t)
        y2 = this.ymin, x2 = x06 + (t = c2) * vx;
    } else if (vy > 0) {
      if (y06 >= this.ymax)
        return null;
      if ((c2 = (this.ymax - y06) / vy) < t)
        y2 = this.ymax, x2 = x06 + (t = c2) * vx;
    }
    if (vx > 0) {
      if (x06 >= this.xmax)
        return null;
      if ((c2 = (this.xmax - x06) / vx) < t)
        x2 = this.xmax, y2 = y06 + (t = c2) * vy;
    } else if (vx < 0) {
      if (x06 <= this.xmin)
        return null;
      if ((c2 = (this.xmin - x06) / vx) < t)
        x2 = this.xmin, y2 = y06 + (t = c2) * vy;
    }
    return [x2, y2];
  }
  _edgecode(x2, y2) {
...
function centroidRingPoint(lambda, phi2) {
  lambda *= radians, phi2 *= radians;
  var cosPhi = cos2(phi2), x2 = cosPhi * cos2(lambda), y2 = cosPhi * sin2(lambda), z = sin2(phi2), cx = y0 * z - z0 * y2, cy = z0 * x2 - x0 * z, cz = x0 * y2 - y0 * x2, m = hypot(cx, cy, cz), w = asin(m), v2 = m && -w / m;
  X2.add(v2 * cx);
  Y2.add(v2 * cy);
  Z2.add(v2 * cz);
  W1 += w;
...
function centroid_default(object2) {
  ...
  return [atan2(y2, x2) * degrees, asin(z / m) * degrees];
}
...
function circle_default() {
  var center2 = constant_default4([0, 0]), radius = constant_default4(90), precision = constant_default4(6), ring, rotate, stream = { point: point6 };
  ...
  circle2.center = function(_) {
    return arguments.length ? (center2 = typeof _ === "function" ? _ : constant_default4([+_[0], +_[1]]), circle2) : center2;
  };
  circle2.radius = function(_) {
    return arguments.length ? (radius = typeof _ === "function" ? _ : constant_default4(+_), circle2) : radius;
  };
  circle2.precision = function(_) {
    return arguments.length ? (precision = typeof _ === "function" ? _ : constant_default4(+_), circle2) : precision;
  };
  return circle2;
}
...
function compareIntersection(a2, b) {
  return ((a2 = a2.x)[0] < 0 ? a2[1] - halfPi2 - epsilon6 : halfPi2 - a2[1]) - ((b = b.x)[0] < 0 ? b[1] - halfPi2 - epsilon6 : halfPi2 - b[1]);
}
  ...
  function clipLine(stream) {
    ...
    return {
      ...
      point: function(lambda, phi2) {
        var point1 = [lambda, phi2], point22, v2 = visible(lambda, phi2), c2 = smallRadius ? v2 ? 0 : code(lambda, phi2) : v2 ? code(lambda + (lambda < 0 ? pi3 : -pi3), phi2) : 0;
        if (!point0 && (v00 = v0 = v2))
          stream.lineStart();
        if (v2 !== v0) {
...
function interpolate_default(a2, b) {
  var x06 = a2[0] * radians, y06 = a2[1] * radians, x12 = b[0] * radians, y12 = b[1] * radians, cy0 = cos2(y06), sy0 = sin2(y06), cy1 = cos2(y12), sy1 = sin2(y12), kx0 = cy0 * cos2(x06), ky0 = cy0 * sin2(x06), kx1 = cy1 * cos2(x12), ky1 = cy1 * sin2(x12), d = 2 * asin(sqrt2(haversin(y12 - y06) + cy0 * cy1 * haversin(x12 - x06))), k2 = sin2(d);
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b0, c0, x12, y12, lambda12, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a2 = a0 + a1, b = b0 + b1, c2 = c0 + c1, m = sqrt2(a2 * a2 + b * b + c2 * c2), phi2 = asin(c2 /= m), lambda22 = abs3(abs3(c2) - 1) < epsilon6 || abs3(lambda04 - lambda12) < epsilon6 ? (lambda04 + lambda12) / 2 : atan2(b, a2), p = project(lambda22, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x06, dy2 = y2 - y06, dz = dy * dx2 - dx * dy2;
      if (dz * dz / d2 > delta2 || abs3((dx * dx2 + dy * dy2) / d2 - 0.5) > 0.3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
        resampleLineTo(x06, y06, lambda04, a0, b0, c0, x2, y2, lambda22, a2 /= m, b /= m, c2, depth, stream);
        stream.point(x2, y2);
...
function conicProjection(projectAt) {
  var phi02 = 0, phi12 = pi3 / 3, m = projectionMutator(projectAt), p = m(phi02, phi12);
...
function azimuthalInvert(angle2) {
  return function(x2, y2) {
    var z = sqrt2(x2 * x2 + y2 * y2), c2 = angle2(z), sc = sin2(c2), cc2 = cos2(c2);
    return [
      atan2(x2 * sc, z * cc2),
      asin(z && y2 * sc / z)
    ];
...
function mercatorProjection(project) {
  var m = projection(project), center2 = m.center, scale2 = m.scale, translate = m.translate, clipExtent = m.clipExtent, x06 = null, y06, x12, y12;
  m.scale = function(_) {
  ...
  function reclip() {
    var k2 = pi3 * scale2(), t = m(rotation_default(m.rotate()).invert([0, 0]));
...
function packEnclose(circles) {
  if (!(n = (circles = array_default2(circles)).length))
    return 0;
  var a2, b, c2, n, aa2, ca3, i, j, k2, sj, sk;
...
function pack_default() {
  ...
  function pack(root) {
    ...
    if (radius) {
      ...
      root.eachBefore(radiusLeaf(defaultRadius2)).eachAfter(packChildren(constantZero, 1)).eachAfter(packChildren(padding, root.r / Math.min(dx, dy))).eachBefore(translateChild(Math.min(dx, dy) / (2 * root.r)));
    }
    return root;
  }
...
var gamma_default = function sourceRandomGamma(source) {
  ...
  function randomGamma(k2, theta) {
    ...
    return function() {
      do {
        do {
          var x2 = randomNormal(), v2 = 1 + c2 * x2;
        } while (v2 <= 0);
        v2 *= v2 * v2;
        var u4 = 1 - source();
  ...
  function arc() {
    ...
    if (!(r1 > epsilon7))
      ...
      if (rc > epsilon7) {
        ...
        if (da2 < pi4 && (oc = intersect(x01, y01, x004, y004, x11, y11, x10, y10))) {
...
function area_default5(x06, y06, y12) {
  var x12 = null, defined = constant_default6(true), context = null, curve = linear_default, output = null;
...
function areaRadial_default() {
  var a2 = area_default5().curve(curveRadialLinear), c2 = a2.curve, x06 = a2.lineX0, x12 = a2.lineX1, y06 = a2.lineY0, y12 = a2.lineY1;
