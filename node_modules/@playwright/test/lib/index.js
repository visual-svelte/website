...
const test = _baseTest.extend({
  ...
  _setupContextOptionsAndArtifacts: [async ({
    ...
    if (hookType(testInfo) === 'beforeAll' && testInfo.status === 'timedOut') {
      ...
      if (pendingCalls) testInfo.error = (0, _util.prependToTestError)(testInfo.error, pendingCalls);
    }
  }, {
    auto: true
  }],
  _contextFactory: async ({
    ...
    if (prependToError) testInfo.error = (0, _util.prependToTestError)(testInfo.error, prependToError);
  },
  context: async ({
    _contextFactory
  }, use) => {
