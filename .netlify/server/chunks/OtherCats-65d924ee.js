...
const OtherCats = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${"outer  svelte-p412a9"}"><div class="${"wrapper svelte-p412a9"}"><h2>Other categories:</h2>
    <div class="${"list-container svelte-p412a9"}">${each(filtered, (post) => {
    return `<a${add_attribute("href", post.href, 0)} style="${"background-color:" + escape(post.color.bg) + "; color:" + escape(post.color.text) + ";"}" class="${"svelte-p412a9"}">${escape(post.text)}
        </a>`;
  })}</div></div>
</div>`;
});
