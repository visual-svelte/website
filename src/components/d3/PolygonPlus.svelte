<script>
  ...
  $: centroid = polygonCentroid(hull); // compute the centroid from the hull
</script>

<p>Polygon area: {polygonArea(data)}</p>
<p>Length of the perimeter: {Math.round(polygonLength(data))}</p>
...
<svg {width} {height}>
  ...
  {#each testPoints as tp}
    <circle
      ...
      cy={yScale(tp[1])}
      r={5}
      fill={polygonContains(hull, tp) ? "green" : "red"}
    />
    <text x={xScale(tp[0]) + 8} y={yScale(tp[1])}
      >{polygonContains(hull, tp) ? "IN" : "OUT"}
    </text>
  {/each}
